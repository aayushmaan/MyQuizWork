...
.factory('QNAFactory', ['$resource', 'baseURL', function ($resource, baseURL) {
    console.log("Factory");
        return $resource(baseURL + "questions/:id",null, {
            'update': {
                method: 'PUT'
            }
        });
...
function useCredentials(credentials) {
  ...
  function destroyUserCredentials() {
    ...
    authFac.login = function(loginData) {
        ...
        .save(loginData,
           function(response) {
              storeUserCredentials({username:loginData.username, token: response.token, role:response.role});
              $rootScope.$broadcast('login:Successful');
            response.role?$state.go('app.create'):$state.go('app.quiz');
           },
           function(response){
              isAuthenticated = false;
